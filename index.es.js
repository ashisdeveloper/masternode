...
export const nextRequest = async (req, res, methods, jwtkey,) => {
...
export const userPermissions = async (api, token = 0, data = {}) => {
    token = token || 0
    if (token) {
...
export const mail = async (smtp = ['', 587, '', ''], from = ['', ''], to = ['', ''], info = ['', ''], reply = ['', '']) => {
    const nodemailer = require("nodemailer");
    let transporter = nodemailer.createTransport({
        host: smtp[0],
...
export const fileUpload = async (apiUrl, dirName, file, options = {}) => {
...
export const fileDelete = async (apiUrl, file) => {
    apiUrl = apiUrl.replace(/\/$/gi, "");
...
export const fileBytesConvert = (bytes, unit = "mb") => {
    let result = 0;
...
export const mysqlSanitizeData = (data) => {
    try {
...
export const mysqlQuery = async (query, { host, user, password, database, port = 3306 }) => {
...
export const mysqlProcedure = async (procName, procAction, data = {}, { host, user, password, database, port = 3306 }, debug = false) => {
    let params = "";
...
export const mysqlTableData = async (reqData, procName, procAction, { host, user, password, database, port = 3306 }) => {
...
export const decrypt = async (hash, key) => {
    const crypto = require("crypto");
    /* const arr = [10, 20, 30, 40, 50, 60, 70, 80, 90, 15, 25, 35, 45, 55, 65, 75];
    let iv = new Int8Array(16);
    for (let i = 0; i < iv.length; i++) iv[i] = arr[i]; */
